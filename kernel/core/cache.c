...
{
#ifdef BRIGHTS_RUST_ENABLED
    /* Rust Murmur3 finalizer for better distribution */
    return rust_hash_u64(key, table_size);
#else
...
{
#ifdef BRIGHTS_RUST_ENABLED
    return rust_hash_string((const uint8_t *)str);
#else
    if (!str) return 0;
...
{
#ifdef BRIGHTS_RUST_ENABLED
    return rust_hash_data((const uint8_t *)data, size);
#else
