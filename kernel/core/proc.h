...
#define BRIGHTS_SCHED_PRIO_HIGH    0
#define BRIGHTS_SCHED_PRIO_NORMAL  1
#define BRIGHTS_SCHED_PRIO_LOW     2
#define BRIGHTS_SCHED_PRIO_CNT     3

/* Per-process scheduling statistics and priority */
typedef struct {
  uint64_t cpu_ticks;           /* accumulated CPU ticks */
  uint64_t sched_count;         /* number of times scheduled */
  uint64_t last_sched_tick;     /* system tick at last dispatch */
  uint64_t total_wait_ticks;    /* estimated total waiting time */
  uint32_t priority;            /* dynamic priority (0=high, 2=low) */
  int32_t  nice;                /* nice value (-20..19, default 0) */
  uint32_t boost_scheduled;     /* scheduler tick when last priority boost happened */
} brights_proc_sched_t;
...
// Reap a zombie process (frees its slot)
