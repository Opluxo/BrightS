...
vfs_mount_t *brights_vfs2_resolve(const char *full_path, const char **rel_path_out)
{
  ...
  if (best) {
    /* Update cache */
    int cache_idx = path_cache_next;
    path_cache[cache_idx].mount = best;
    vfs_str_copy(path_cache[cache_idx].path, sizeof(path_cache[cache_idx].path), full_path);
    vfs_str_copy(path_cache[cache_idx].rel_path, sizeof(path_cache[cache_idx].rel_path), best_rel_path);
    path_cache_next = (cache_idx + 1) % PATH_CACHE_SIZE;

    if (rel_path_out) *rel_path_out = path_cache[cache_idx].rel_path;
  }
