...
#else
/* i386 user mode implementation */
#include "../arch/i386/gdt.h"

extern void brights_enter_user(void *eip, void *esp, uint32_t cr3);

/* Embedded user test program - minimal i386 ELF */
static const uint8_t embedded_init_elf[] = {
  ...
  0x00, 0x80, 0x04, 0x08, 0x00, 0x80, 0x04, 0x08, 0x7b, 0x00, 0x00, 0x00,
  0x7b, 0x00, 0x00, 0x00, 0x05, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x00,
  0xb8, 0x02, 0x00, 0x00, 0x00, 0xbb, 0x01, 0x00, 0x00, 0x00, 0xb9, 0x74,
  0x80, 0x04, 0x08, 0xba, 0x07, 0x00, 0x00, 0x00, 0xcd, 0x80, 0xb8, 0x05,
  0x00, 0x00, 0x00, 0x31, 0xdb, 0xcd, 0x80, 0xf4, 0x48, 0x65, 0x6c, 0x6c,
  0x6f, 0x21, 0x0a
};
static const uint32_t embedded_init_elf_size = 123;
...
#else
  /* Set up TSS for user mode transitions (GDT already has user segments from gdt_init) */
  static uint8_t kstack_buf[8192] __attribute__((aligned(16)));
  uint32_t kstack_top = (uint32_t)(uintptr_t)&kstack_buf[sizeof(kstack_buf)];
  brights_tss_init(kstack_top);
  brights_gdt_init();
